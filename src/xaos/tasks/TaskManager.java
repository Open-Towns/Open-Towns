...
public final class TaskManager implements Externalizable {
    ...
    public static int MAX_CONTAINER_HAUL_PER_TURN = 64; // Número de containers a chequear, para ver si hay items maols
                                                        // dentro
    ...
    private ArrayList<TaskManagerItem> taskItemsTemp; // Se usa este array para meter las tareas y que no se coloquen
                                                      // asíncronamente en el "bueno"
    private ArrayList<Action> customActions;
    private ArrayList<Action> customActionsTemp; // Se usa este array para meter las actions y que no se coloquen
                                                 // asíncronamente en el "bueno"
    private ArrayList<Action> customActionsWait; // Se usa este array cuando se saca una action que no se puede hacer
                                                 // pero no hay que eliminarla (performance)
    private int haulIndex;
    private int containerIndex;
    private boolean reCheckMinePlaces;

    private HashMap<String, Integer> hmItemsOnRegularQueue; // Se usa para que el production panel sepa cuantos items
                                                            // hay en cola
    private HashMap<String, Integer> hmItemsOnAutomatedQueue; // Se usa para que el production panel sepa cuantos items
                                                              // hay automated mode
    ...
    private boolean isDuplicateAction(Action newAction, Action action) {
        ...
        if (newAction.getDestinationPoint() != null && action.getDestinationPoint() != null
                && newAction.getDestinationPoint().equals(action.getDestinationPoint())) {
            return true;
        }
    ...
    private int checkQueueAction(ActionManagerItem ami, int iNumOnAPS) {
        if (ami.getGeneratedItem() != null) {
            int iWorld = Item.getNumItems(UtilsIniHeaders.getIntIniHeader(ami.getGeneratedItem()), false,
                        ...
                        if (qi.getType() == QueueItem.TYPE_MOVE) {
                            ...
                            if (alList != null && alList.size() > 0) {
                                ...
                                for (int t = 0; t < alList.size(); t++) {
                                    iTmp += Item.getNumItems(UtilsIniHeaders.getIntIniHeader(alList.get(t)), true,
                                            Game.getWorld().getRestrictHaulEquippingLevel());
                                }
                                if (iTmp > 0) {
                                    bAnyItem = true;
                            ...
                            if (alList != null && alList.size() > 0) {
                                ...
                                int iTmp = 0;
                                for (int t = 0; t < alList.size(); t++) {
                                    iTmp += Item.getNumItems(UtilsIniHeaders.getIntIniHeader(alList.get(t)), false,
                                            Game.getWorld().getRestrictHaulEquippingLevel());
                                }
                                if (iTmp > 0) {
                                    bAnyItem = true;
    ...
    public void executeAll(boolean bOnlyDirect) {
        // System.out.println (customActions.size () + ", " + customActionsTemp.size ()
        // + ", " + customActionsWait.size ());
        // Miramos si tenemos tareas automáticas del panel de producción, sólo lo
        // hacemos cada FPS_INGAME turnos
            ...
            synchronized (hmItemsOnAutomatedQueue) {
                ...
                while (it.hasNext()) {
                    ...
                    if (iValue.intValue() > 0) {
                        ...
                        // Prerequisitos OK, metemos las qué podemos hacer (en base a moves y picks
                        // ahora mismo)
        ...
        // Tiene que hacerse lo primero de todo, ya que si no podrían pasar tareas sin
        // hotpoint (tareas directas) al método de asignar tareas normales
        synchronized (taskItemsTemp) {
        ...
        synchronized (alProductionToRemove) {
            synchronized (customActionsTemp) {
                ...
                // Primero metemos 4 de las wait (esto es para performance, si hay 2000 acciones
                // no las chequeará todas cada vez)
                for (int i = 0; i < 4; i++) {
                ...
                if (alHotpoints != null && alHotpoints.size() > 0) {
                    for (int h = 0; h < alHotpoints.size(); h++) {
                        ...
                        if (!alHotpoints.get(h).isFinished()) {
                            alHotpoints.get(h).setPlaces(
                                    Task.getAccessingPoints(hPoint3D.x, hPoint3D.y, hPoint3D.z, iTaskType), true);
                        }
                    }
                }
            }
    ...
    public void executeTaskImmediately(Task task) {
        ...
        TaskManagerItem item = new TaskManagerItem(task);
        executeDirectTask(item);
    }
    ...
    private void executeDirectTask(TaskManagerItem item) {
        ...
        if (item.getTask().getTask() == Task.TASK_DESTROY_BUILDING) {
        ...
        } else if (item.getTask().getTask() == Task.TASK_TURN_OFF_NON_STOP
                || item.getTask().getTask() == Task.TASK_TURN_ON_NON_STOP) {
            ...
            // int id = Terrain.getTileHeightID (item.getTask ().getPointIni ().x,
            // item.getTask ().getPointIni ().y, item.getTask ().getPointIni ().z);
            // cell.getTerrain ().setIniHeader (item.getTask ().getParameter ());
            // cell.getTerrain ().setTileSetCoordinates (item.getTask ().getParameter () +
            // MapGenerator.SLOPES_INIHEADER[id], item.getTask ().getParameter ());
            // cell.getTerrain ().setTileSize (item.getTask ().getParameter () +
            // MapGenerator.SLOPES_INIHEADER[id], item.getTask ().getParameter ());
            ...
            Game.getWorld().addFluidCellToProcess(item.getTask().getPointIni().x, item.getTask().getPointIni().y,
                    item.getTask().getPointIni().z, false);
            ...
            if (imi != null) {
                ...
                if (sBuildingIniHeader != null) {
                    // Buscamos edificio donde se pueda construir, si tiene la cola llena seguimos a
                    // ver si encontramos uno sin cola
                    Building building;
                    ...
                    if (iBuildingIndex == -1) {
                        ...
                        if (bmi == null) {
                            Log.log(Log.LEVEL_ERROR, Messages.getString("TaskManager.1") + sBuildingIniHeader + "]", //$NON-NLS-1$ //$NON-NLS-2$
                                    getClass().toString());
                        } else {
                            MessagesPanel.addMessage(MessagesPanel.TYPE_ANNOUNCEMENT,
                                    Messages.getString("TaskManager.3") + imi.getName() //$NON-NLS-1$
                                            + Messages.getString("TaskManager.4") + bmi.getName() //$NON-NLS-1$
                    ...
                    Log.log(Log.LEVEL_ERROR, Messages.getString("TaskManager.6") + sItemIniHeader //$NON-NLS-1$
                            + Messages.getString("TaskManager.7"), getClass().toString()); //$NON-NLS-1$
                }
            } else {
                Log.log(Log.LEVEL_ERROR, Messages.getString("TaskManager.8") + sItemIniHeader + "]", //$NON-NLS-1$ //$NON-NLS-2$
                        getClass().toString());
            }
            ...
            //
            // //Stockpile.enableAll (Integer.parseInt (item.getTask ().getParameter ()),
            // item.getTask ().getParameter2 ());
            // item.getTask ().setFinished (true);
            // } else if (item.getTask ().getTask () == Task.TASK_STOCKPILE_DISABLE_ALL) {
            // // STOCKPILE DISABLE ALL
            //
            // //Stockpile.disableAll (Integer.parseInt (item.getTask ().getParameter ()),
            // item.getTask ().getParameter2 ());
            // item.getTask ().setFinished (true);
            ...
            // Stockpile stockpile = Stockpile.getStockpile (item.getTask ().getPointIni
            // ().toPoint3DShort ());
            // if (stockpile != null && !stockpile.getType ().contains (item.getTask
            // ().getParameter ())) {
            ...
            // Game.getWorld ().addItemToBeHauled (World.getCell (stockpile.getPoints ().get
            // (h)).getItem ());
            // }
            // }
            // item.getTask ().setFinished (true);
            ...
            // Container container = Game.getWorld ().getContainer (Integer.parseInt
            // (item.getTask ().getParameter ()));
            // if (container != null) {
            ...
            // }
            // item.getTask ().setFinished (true);
        } else if (item.getTask().getTask() == Task.TASK_CONVERT_TO_CIVILIAN
                || item.getTask().getTask() == Task.TASK_CONVERT_TO_SOLDIER) {
            ...
            if (citizen != null && ((citizen.getSoldierData().isSoldier()
                    && item.getTask().getTask() == Task.TASK_CONVERT_TO_CIVILIAN)
                    || (!citizen.getSoldierData().isSoldier()
                            && item.getTask().getTask() == Task.TASK_CONVERT_TO_SOLDIER))) {
                if (item.getTask().getTask() == Task.TASK_CONVERT_TO_SOLDIER) {
                    ...
                    Game.getWorld().getCitizenGroups().getCitizensWithoutGroup()
                            .add(Integer.valueOf(citizen.getID()));
                    // Borramos sus jobs
            ...
            if (citizen != null) {
                ...
                if (citizen.getSoldierData().getState() != iState || (iState == SoldierData.STATE_IN_A_GROUP
                        && citizen.getSoldierData().getGroup() != item.getTask().getPointIni().x)) {
                    citizen.getSoldierData().setState(iState, item.getTask().getPointIni().x, citizen.getID());
                    if (iState == SoldierData.STATE_BOSS_AROUND) {
                        citizen.getSoldierData()
                                .setCounter(Utils.getRandomBetween(0, SoldierData.COUNTER_MAX_BOSS_AROUND - 1));
                    }

                    // Tutorial flow
                    if (iState == SoldierData.STATE_GUARD) {
                        Game.updateTutorialFlow(TutorialTrigger.TYPE_INT_ICONHIT,
                                TutorialTrigger.ICON_INT_LIVINGS_GUARD, null);
                    } else if (iState == SoldierData.STATE_PATROL) {
                        Game.updateTutorialFlow(TutorialTrigger.TYPE_INT_ICONHIT,
                                TutorialTrigger.ICON_INT_LIVINGS_PATROL, null);
                    } else if (iState == SoldierData.STATE_BOSS_AROUND) {
                        Game.updateTutorialFlow(TutorialTrigger.TYPE_INT_ICONHIT,
                                TutorialTrigger.ICON_INT_LIVINGS_BOSS, null);
                    }
                }
            }
    ...
    private void removeFinishedTasks() {
        ...
        for (int i = (taskItems.size() - 1); i >= 0; i--) {
            ...
            if (item.getTask().isFinished()) {
                ...
                // Si era tarea de haul reseteamos el contador de turnos, para que vuelva a
                // chequear el haul sin esperar MAX_HAUL_TURNS turnos
                // if (tmi != null && tmi.getTask () != null && (tmi.getTask ().getTask () ==
                // Task.TASK_HAUL || tmi.getTask ().getTask () == Task.TASK_PUT_IN_CONTAINER)) {
    ...
    private void assignTasks() {
        ...
        // assignFightTasks ();
        // Obtenemos aldeanos inactivos (y que no estén cargando nada) y los metemos en
        // una hash, la key será el número de zona A*
        ...
        for (int i = 0; i < citizens.size(); i++) {
            citizen = (Citizen) World.getLivingEntityByID(citizens.get(i));
            if (!citizen.isWaitingForPath() && citizen.isIdle() && citizen.getCarrying() == null
                    && citizen.getCarryingLiving() == null && citizen.getCitizenData().getSleep() > 0) {
                // Tenemos aldeano sin tarea, miramos la zona A* y lo metemos en la hash
                Integer iZID = Integer
                        .valueOf(World.getCells()[citizen.getX()][citizen.getY()][citizen.getZ()].getAstarZoneID());
        ...
        Game.iError = 670;
        // Nos hemos "pulido" todas las tareas, miramos si quedan aldeanos libres para
        // las tareas mine/dig, move_caravan, build_building, feed_animals, custom y de
        // haul
        iLibres = getNumCitizens(hmCitizensSinTarea);
        if (iLibres == 0) {
            return;
        ...
        int iMineDigPriority = ActionPriorityManager
                .getPriorityValueByPriorityID(ActionPriorityManager.PRIORITY_MINE_DIG);
        int iHaulPriority = ActionPriorityManager.getPriorityValueByPriorityID(ActionPriorityManager.PRIORITY_HAUL);
        int iTradingPriority = ActionPriorityManager
                .getPriorityValueByPriorityID(ActionPriorityManager.PRIORITY_TRADING);
        int iBuildingsPriority = ActionPriorityManager
                .getPriorityValueByPriorityID(ActionPriorityManager.PRIORITY_BUILD_BUILDINGS);
        int iFeedAnimalsPriority = ActionPriorityManager
                .getPriorityValueByPriorityID(ActionPriorityManager.PRIORITY_FEED_ANIMALS);
        ...
        while (iPriorityIndex < ActionPriorityManager.getPrioritiesListSize()) {
            if (iPriorityIndex == iMineDigPriority) {
                assignNormalTasks(hmCitizensSinTarea, citizens, TASK_TYPE_MINEDIG,
                        ActionPriorityManager.PRIORITY_MINE_DIG);
                ...
                assignNormalTasks(hmCitizensSinTarea, citizens, TASK_TYPE_FEED_ANIMALS,
                        ActionPriorityManager.PRIORITY_FEED_ANIMALS);
                iFeedAnimalsPriority = -2; // Esto servirá para saber que ya la hemos tratado
                iLibres = getNumCitizens(hmCitizensSinTarea);
            } else if (iPriorityIndex == iTradingPriority) {
                assignNormalTasks(hmCitizensSinTarea, citizens, TASK_TYPE_MOVE_TO_CARAVAN,
                        ActionPriorityManager.PRIORITY_TRADING);
                iTradingPriority = -2; // Esto servirá para saber que ya la hemos tratado
                iLibres = getNumCitizens(hmCitizensSinTarea);
            } else if (iPriorityIndex == iBuildingsPriority) {
                assignNormalTasks(hmCitizensSinTarea, citizens, TASK_TYPE_BUILD_BUILDING,
                        ActionPriorityManager.PRIORITY_BUILD_BUILDINGS);
     ...
     * @return
     */
    private void assignNormalTasks(HashMap<Integer, ArrayList<Citizen>> hmCitizensSinTarea, ArrayList<Integer> citizens,
            int iTaskType, String sPriorityID) {
        TaskManagerItem item;
        ...
        fortasks: for (int i = 0; i < taskItems.size(); i++) {
            if (hmCitizensSinTarea.size() == 0) {
                return;
            }
            item = taskItems.get(i);
            ...
            if (item.getTask().getTask() == Task.TASK_WEAR || item.getTask().getTask() == Task.TASK_WEAR_OFF
                    || item.getTask().getTask() == Task.TASK_AUTOEQUIP) {
                continue;
            }
            ...
            if (iTaskType == TASK_TYPE_OTHERS) {
                ...
                if (item.getTask().getTask() == Task.TASK_MINE || item.getTask().getTask() == Task.TASK_MINE_LADDER
                        || item.getTask().getTask() == Task.TASK_BUILD
                        || item.getTask().getTask() == Task.TASK_FOOD_NEEDED
                        || item.getTask().getTask() == Task.TASK_MOVE_TO_CARAVAN) {
                    continue;
                }
            } else {
                // Miramos sólo las tareas del tipo pasado
                if (iTaskType == TASK_TYPE_MINEDIG) {
                    if (item.getTask().getTask() != Task.TASK_MINE
                            && item.getTask().getTask() != Task.TASK_MINE_LADDER) {
                        continue;
                    }
            ...
            // Miramos si es tarea de construcción, en ese caso no asignamos aldeano si no
            // hay items (o livings) en el mundo
            // System.out.println (item.getTask ().getParameter () + ", " + item.getTask
            // ().getParameter2 ());
            if (item.getTask().getTask() == Task.TASK_BUILD) {
                Game.iError = 6671;
                ...
                if (alPrerequisites == null || alPrerequisites.size() == 0) {
                    ...
                    for (int it = 0; it < alPrerequisites.get(0).length; it++) {
                        iNumItems += Item.getNumItems(alPrerequisites.get(0)[it], false,
                                Game.getWorld().getRestrictHaulEquippingLevel());
                        if (iNumItems != 0) {
                            break;
                        }
                    ...
                    if (iNumItems <= 0) {
                        ...
                        // Si hay materiales miramos que nos vayan bien como prerequisito y que estén en
                        // el mismo A*ZoneID que el edificio
                        boolean bMatsEnZona = false;
                        for (int it = 0; it < alPrerequisites.size(); it++) {
                            bMatsEnZona = (Item.searchItem(true, building.getCoordinates(), alPrerequisites.get(it),
                                    false, Item.SEARCH_FALSE, Item.SEARCH_DOESNTMATTER, null,
                                    Game.getWorld().getRestrictHaulEquippingLevel()) != null);
                            if (bMatsEnZona) {
                                break;
                            }
                        }
                ...
                if (bItemPrerequisitesDone) {
                    ...
                    if (alPrerequisites != null && alPrerequisites.size() > 0) {
                        // Hay prerequisitos living, antes de asignar la tarea miramos que exista alguno
                        // de esos livings en la zona del edificio
                        boolean bLivingEnZona = false;
                        for (int li = 0; li < alPrerequisites.size(); li++) {
                            bLivingEnZona = (LivingEntity.searchLiving(building.getCoordinates(),
                                    alPrerequisites.get(li), false, null) != null);
                            if (bLivingEnZona) {
                                break;
                            }
                        }
                ...
                if (iItemID != -1) {
                    ...
                    if (p3dMilitar == null) {
                        // No está actualmente en el mundo, quizá en el carrying de algún aldeano, de
                        // momento pasamos
                        continue;
                    } else {
                        // El item existe, podemos asignar tarea a algún aldeano (si está en la misma
                        // zona que la caravana)
                    ...
                    if (Item.searchItem(true, leCaravan.getCoordinates(), aiHeaders, true, Item.SEARCH_FALSE,
                            Item.SEARCH_DOESNTMATTER, null, Game.getWorld().getRestrictHaulEquippingLevel()) == null) {
                ...
                if (leToFeed == null) {
                    ...
                    // caso (ha muerto) eliminamos la tarea
                    Citizen cit;
                    for (int c = 0; c < World.getCitizenIDs().size(); c++) {
                        cit = (Citizen) World.getLivingEntityByID(World.getCitizenIDs().get(c));
                        if (cit != null && cit.getCarryingLiving() != null
                                && cit.getCarryingLiving().getID() == iLivingID) {
                ...
                if (lemi.getFoodNeeded() == null) {
                    // Curiosamente no tiene feed, quizá se ha modificado el .xml y el animal es
                    // antiguo, pacasa
                    item.getTask().setFinished(true);
                    continue;
                }

                Point3DShort p3dItem = Item.searchItem(true, leToFeed.getCoordinates(), lemi.getFoodNeeded(), true,
                        Item.SEARCH_FALSE, Item.SEARCH_DOESNTMATTER, null,
                        Game.getWorld().getRestrictHaulEquippingLevel());
                if (p3dItem == null) {
                    continue;
                }
            ...
            while (iCitizens < iMaxCitizens && !bTaskEnded) {
                ...
                iIndex = 0; // Marcará el índice de aldeano que está más cerca y que esté en la misma zona
                            // que alguno de sus places
                int iHPIndex = 0;
                HotPoint hotPoint;
                ...
                while (!bHotPointEncontrado && !bTaskEnded) {
                    ...
                    // Miramos que NO esté terminado y que ningún aldeano ya lo esté haciendo y que
                    // las places del hotpoint sean accesible por aldeanos
                    boolean bHotpointLibre = !hotPoint.isFinished();

                    if (bHotpointLibre) {
                        Game.iError = 66731;
                        // Miramos las places del hotpoint a ver si hay alguna accesible y con aldeanos
                        // que puedan llegar
                        ...
                        for (int n = 0; n < hotPoint.getPlaces().size(); n++) {
                            ...
                            if (iASZID != -1) {
                                ...
                                if (alCitTmp != null && alCitTmp.size() > 0
                                        && Citizen.isCellAllowed(hPoint3D.x, hPoint3D.y, hPoint3D.z)) {
                                    iPlacesAccesibles++;
                                }
                            }
                        }
                            ...
                            for (int n = 0; n < citizens.size(); n++) {
                                ...
                                if (citizen.getHotPointIndex() == iHPIndex) {
                                    if (citizen.getCurrentTask() != null
                                            && citizen.getCurrentTask().getID() == item.getTask().getID()) {
                    ...
                    if (bHotpointLibre) { // Hotpoint sin terminar y sin nadie haciéndolo, le metemos el aldeano
                                          // INACTIVO más cercano con A*ZoneID igual a alguna de sus places
                        Game.iError = 6674;

                        bHotPointEncontrado = true;
                        hPoint3D = hotPoint.getHotPoint();

                        // Para evitar stucks raros primero miramos esta casuistica (haul con hotpoint
                        // distinto de pointini)
                        if (item.getTask().getTask() == Task.TASK_HAUL
                                || item.getTask().getTask() == Task.TASK_MOVE_AND_LOCK
                                || item.getTask().getTask() == Task.TASK_PUT_IN_CONTAINER) {
                            if (!hPoint3D.equals(item.getTask().getPointIni())
                                    || (World.getCell(hPoint3D).getAstarZoneID() == -1)) {
                                item.getTask().setFinished(true);
                                continue fortasks;
                            }
                        }
                        ...
                        // Si hay places con distintos zoneIDs uniremos los 2 (o N) arrays de aldeanos
                        // en uno
                        ...
                        for (int c = 0; c < hotPoint.getPlaces().size(); c++) {
                            ...
                            ArrayList<Citizen> alCits = hmCitizensSinTarea.get(Integer
                                    .valueOf(World.getCells()[p3dPlaces.x][p3dPlaces.y][p3dPlaces.z].getAstarZoneID()));
                        ...
                        // Ya tenemos el array con todos los aldeanos que comparten A*ZoneID con las
                        // places
                        if (alCitizensSinTarea.size() == 0) {
                            ...
                            if (iIndex != -1) {
                                ...
                                Integer iZoneID = Integer
                                        .valueOf(World.getCells()[citizen.getX()][citizen.getY()][citizen.getZ()]
                                                .getAstarZoneID());
     ...
     * @param iIndex
     */
    private Action removeAction(int iIndex, boolean backToTemp,
            HashMap<String, ArrayList<Integer>> hmNonAvailableActions) {
        return removeAction(iIndex, backToTemp, true, hmNonAvailableActions);
    }

    private Action removeAction(int iIndex, boolean backToTemp, boolean removeFromProdPanel,
            HashMap<String, ArrayList<Integer>> hmNonAvailableActions) {
     ...
     * @param hmCitizensSinTarea
     */
    private int assignCustomActions(HashMap<Integer, ArrayList<Citizen>> hmCitizensSinTarea, int iLibres,
            int priorityIndex) {
        ...
        // Hash de action(String) / lista de ASZID donde no se puede hacer (performance
        // trick)
        hmNonAvailableActions.clear();

        // Recorremos las actions
        synchronized (customActions) {
            while (iActionsProcesadas < MAX_CUSTOM_ACTIONS_PER_TURN_AND_PRIORITY_LEVEL
                    && iIndex < customActions.size()) {
                action = customActions.get(iIndex);
                if (priorityIndex != -1
                        && ActionPriorityManager.getPriorityValueByActionID(action.getId()) != priorityIndex) {
                    iIndex++;
                    continue;
                }
                        ...
                        if (queueItem.getType() == QueueItem.TYPE_CREATE_ITEM) {
                            ...
                            if (imi == null) {
                                Log.log(Log.LEVEL_ERROR,
                                        Messages.getString("TaskManager.10") + queueItem.getValue() + "]", //$NON-NLS-1$ //$NON-NLS-2$
                                        getClass().toString());
                            ...
                            if (alItems == null || alItems.size() == 0) {
                                Log.log(Log.LEVEL_ERROR,
                                        Messages.getString("TaskManager.11") + " [" + queueItem.getValue() + "]", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                                        getClass().toString());
                                removeAction(iIndex, false, null);
                                bBadAction = true;
                                break;
                ...
                // Me aseguro que el primer "pick/pickLiving" y TODOS los "move" estén
                // disponibles para el aldeano (de momento sólo pillo valores, más adelante lo
                // miro)
                String sPickItemLiving = null;
                ArrayList<String> alMoveITem = new ArrayList<String>();
                int iFirstPickItemOrLiving = 0; // 1 - lo primero es un pick item, 2 - lo primero es un pick_living, 0 -
                                                // no hay picks
                ...
                // Cola chequeada, miramos que los items tengan el mismo A*zoneID y que existan
                // (y operativas las de move)
                ...
                int iCitASZID;
                for (int i = 0; i < citizens.size(); i++) {
                    iCitASZID = World.getCell(World.getLivingEntityByID(citizens.get(i)).getCoordinates())
                            .getAstarZoneID();
                ...
                if (alMoveITem.size() > 0) {
                    // Miraremos según los aldeanos (NO miramos los libres, que quizá no hay
                    // ninguno, miramos TODOS)
                    ...
                    while (iMovesEncontrados < alMoveITem.size() && alASZIDAMirarCoordinates.size() > 0) {
                        ...
                        for (int i = 0; i < alMoveITem.size(); i++) {
                            ...
                            // Point3D puntoMoveItem = Item.searchItem (p3d, itemToSearch, false,
                            // Item.SEARCH_TRUE, Item.SEARCH_TRUE, null, true); // Buscamos ordenadamente
                            // (el último true)
                            Point3DShort puntoMoveItem = Item.searchItem(false, p3d,
                                    UtilsIniHeaders.getIntsArray(itemToSearch), false, Item.SEARCH_DOESNTMATTER,
                                    Item.SEARCH_TRUE, null, true, false,
                                    Game.getWorld().getRestrictHaulEquippingLevel()); // Buscamos ordenadamente,
                                                                                      // evitamos containers
                    ...
                    if (sMoveMissing != null) {
                        if (!action.isSilent()) {
                            ...
                            if (moveItems.size() > 0) {
                                ...
                                MessagesPanel.addMessage(MessagesPanel.TYPE_ANNOUNCEMENT,
                                        ami.getName() + Messages.getString("TaskManager.5") + sName //$NON-NLS-1$
                                                + Messages.getString("TaskManager.2"), //$NON-NLS-1$
                                        ColorGL.RED);
                            }
                        }
                ...
                // Si llega aquí es que los moves están OK, miramos que el primero de la lista
                // no esté en uso, en ese caso saltamos la tarea por el momento
                if (alMoveITem.size() > 0) {
                    ...
                    while (itAldeanos.hasNext()) {
                        ...
                        if (alCits != null && alCits.size() > 0) {
                            // if (Item.searchItem (alCits.get (0).getCoordinates (), itemsToSearch, false,
                            // Item.SEARCH_TRUE, Item.SEARCH_TRUE, alItemsInUse) != null) {
                            if (Item.searchItem(true, alCits.get(0).getCoordinates(),
                                    UtilsIniHeaders.getIntsArray(itemsToSearch), false, Item.SEARCH_DOESNTMATTER,
                                    Item.SEARCH_TRUE, alItemsInUse,
                                    Game.getWorld().getRestrictHaulEquippingLevel()) != null) {
                                bPrimerMoveOK = true;
                                break;
                            }
                ...
                if (sPickItemLiving != null) {
                    ...
                    if (iFirstPickItemOrLiving == 1) { // Items
                        ...
                        for (int i = 0; i < aiPicks.length; i++) {
                            if (Item.getNumItems(aiPicks[i], false,
                    ...
                    if (alMoveITem.size() > 0) {
                        ...
                        if (iFirstPickItemOrLiving == 1) {
                            ...
                            puntoPickItemLiving = Item.searchItem(false, p3dMove, aiPicks, false, Item.SEARCH_FALSE,
                                    Item.SEARCH_DOESNTMATTER, null, Game.getWorld().getRestrictHaulEquippingLevel());
                        } else {
                            // Lo primero es una friendly
                            LivingEntity le = LivingEntity.searchLiving(p3dMove, UtilsIniHeaders.getIntsArray(alPicks),
                                    false, null);
                            if (le != null) {
                        ...
                        if (p3dMoveTerrain != null) {
                            ...
                            if (cellTerrain.isMined()) {
                                ...
                                if (p3dMoveTerrain.z > 0) {
                                    cellTerrain = World.getCell(p3dMoveTerrain.x, p3dMoveTerrain.y,
                                            p3dMoveTerrain.z - 1);
                                    alCits = hmCitizensSinTarea.get(cellTerrain.getAstarZoneID());
                                    // } else {
                                    // No se puede acceder
                            ...
                            if (alCits != null && alCits.size() > 0) {
                                ...
                                if (iFirstPickItemOrLiving == 1) {
                                    puntoPickItem = Item.searchItem(false, alCits.get(0).getCoordinates(), aiPicks,
                                            false, Item.SEARCH_FALSE, Item.SEARCH_DOESNTMATTER, null,
                                            Game.getWorld().getRestrictHaulEquippingLevel());
                                } else {
                                    LivingEntity le = LivingEntity.searchLiving(alCits.get(0).getCoordinates(),
                                            UtilsIniHeaders.getIntsArray(alPicks), false, null);
                                    if (le != null) {
                ...
                if (p3dMoveTerrain != null) {
                    ...
                    // Ni move ni pick..... cualquier aldeano nos vale (que esté en el mismo A*ZID
                    // del destination !)
                    if (action.getDestinationPoint() != null
                            && World.getCell(action.getDestinationPoint()).getAstarZoneID() != -1) {
                        alCits = hmCitizensSinTarea
                                .get(Integer.valueOf(World.getCell(action.getDestinationPoint()).getAstarZoneID()));
                        if (alCits != null && alCits.size() > 0) {
                            p3dDestination = alCits.get(0).getCoordinates();
                        }

                        if (p3dDestination == null) {
                            // No hay aldeanos en la zona, miramos aldeanos en otras zonas adyacentes para
                            // ver si pueden construir desde otra celda
                            Point3DShort p3dNeighbour;
                            int iASZIDNeighbour;
                            bucleNeighbours: for (int x = -1; x <= 1; x++) {
                                for (int y = -1; y <= 1; y++) {
                                    for (int z = -1; z <= 1; z++) {
                                        p3dNeighbour = Point3DShort.getPoolInstance(action.getDestinationPoint().x + x,
                                                action.getDestinationPoint().y + y, action.getDestinationPoint().z + z);
                ...
                if (alCits != null && alCits.size() > 0) {
                    ...
                    if (iIndexCit != -1) {
                        ...
                        iLibres--;
                        hmCitizensSinTarea
                                .put(Integer.valueOf(World.getCell(citizen.getCoordinates()).getAstarZoneID()), alCits);
                        ...
                        // Tarea metida, la metemos en las non-available para que espere al menos 1
                        // turno (por los <locks>)
                        addToNonAvailableHash(action, hmNonAvailableActions);
                    } else {
    ...
    private int assignCreateTasks(HashMap<Integer, ArrayList<Citizen>> hmCitizensSinTarea, int iLibres) {
        ...
        // Recorremos todos los edificios a ver si hay alguno con tarea de construcción
        // y ningún aldeano haciéndola
        Building building;
        ArrayList<Citizen> alCits;
        breakBuildings: for (int i = 0; i < World.getBuildings().size(); i++) {
            ...
            if (building.isOperative() && building.hasItemsInQueue()) {
                ...
                // Edificio con tareas, primero miramos si hay aldeanos en la zona y después
                // miramos si no se está haciendo por otro aldeano
                ...
                for (int c = 0; c < citizens.size(); c++) {
                    ...
                    if (citizen.getCurrentTask() != null
                            && (citizen.getCurrentTask().getTask() == Task.TASK_CREATE_AND_PLACE)) {
                ...
                if (alprerequisites != null && alprerequisites.size() > 0) {
                    if (Item.searchItem(true, p3dEntrance, UtilsIniHeaders.getIntsArray(alprerequisites), false,
                            Item.SEARCH_FALSE, Item.SEARCH_DOESNTMATTER, null,
                            Game.getWorld().getRestrictHaulEquippingLevel()) == null) {
                        continue breakBuildings;
                    }
        ...
        for (int i = 0; i < taskItems.size(); i++) {
            ...
            if (task.getTask() == Task.TASK_WEAR) {
                ...
                if (task.getParameter() == null || task.getPointIni() == null) {
                    // Raro, pero a la peña le peta en algún sitio (quizá el aldeano está muerto
                    // cuando clican en el menú de equipar o algo)
                    task.setFinished(true);
                    continue;
                }
                ...
                if (citizen != null) {
                    Game.iError = 916;
                    if (World.getCell(citizen.getCoordinates()).getAstarZoneID() == World.getCell(p3dItem)
                            .getAstarZoneID()) {
                        Game.iError = 917;
                        // Aldeano en la zona
                        if (citizen.isIdle()) {
                            ...
                            ArrayList<Citizen> alCits = hmCitizensSinTarea
                                    .get(World.getCell(citizen.getCoordinates()).getAstarZoneID());
                            if (alCits != null) {
                                ...
                                hmCitizensSinTarea.put(World.getCell(citizen.getCoordinates()).getAstarZoneID(),
                                        alCits);
                            }
                        } else {
                            Game.iError = 920;
                            // No ocioso, si no está comiendo ni durmiendo ni equipándose lo sacamos de su
                            // tarea y le metemos la nueva
                            if (citizen.getCurrentTask() == null || (citizen.getCurrentTask().getTask() != Task.TASK_EAT
                                    && citizen.getCurrentTask().getTask() != Task.TASK_SLEEP
                                    && citizen.getCurrentTask().getTask() != Task.TASK_WEAR
                                    && citizen.getCurrentTask().getTask() != Task.TASK_WEAR_OFF
                                    && citizen.getCurrentTask().getTask() != Task.TASK_AUTOEQUIP
                                    && citizen.getCurrentTask().getTask() != Task.TASK_DROP
                                    && citizen.getCurrentTask().getTask() != Task.TASK_HEAL)) {
                                removeCitizen(citizen);
                                citizen.setCurrentTask(task);
                            }
                        }
                        ...
                        task.setFinished(true);
                        MessagesPanel.addMessage(MessagesPanel.TYPE_ANNOUNCEMENT,
                                citizen.getCitizenData().getFullName() + Messages.getString("TaskManager.0"), //$NON-NLS-1$
                                ColorGL.ORANGE);
                    }
                }
                ...
                // Truquito el pointini.x marca el ID de aldeano, el pointini.y marca el
                // location del item
                ...
                if (citizen != null) {
                    if (citizen.isIdle()) {
                        ...
                        // No ocioso, si no está comiendo ni bebiendo ni desequipándose lo sacamos de su
                        // tarea y le metemos la nueva
                        if (citizen.getCurrentTask() == null || (citizen.getCurrentTask().getTask() != Task.TASK_EAT
                                && citizen.getCurrentTask().getTask() != Task.TASK_SLEEP
                                && citizen.getCurrentTask().getTask() != Task.TASK_WEAR
                ...
                // Tarea virtual de autoequip, buscamos al aldeano, nos da igual si está ocupado
                // o no
                ...
                if (citizen != null) {
                    ...
                    if (citizen.isIdle()) {
                        ...
                        // No ocioso, si no está comiendo ni durmiendo ni curándose ni equipándose lo
                        // sacamos de su tarea y le metemos la nueva
                        if (citizen.getCurrentTask() == null || (citizen.getCurrentTask().getTask() != Task.TASK_EAT
                                && citizen.getCurrentTask().getTask() != Task.TASK_SLEEP
                                && citizen.getCurrentTask().getTask() != Task.TASK_WEAR
                                && citizen.getCurrentTask().getTask() != Task.TASK_WEAR_OFF
        ...
        for (int i = containerIndex; i < MAX; i++) {
            ...
            if (container.isWrongItemsInside()) {
                ...
                if (itemContainer != null) {
                    int iZoneContainer = Integer
                            .valueOf(World.getCell(itemContainer.getCoordinates()).getAstarZoneID());
                    ArrayList<Citizen> alCits = hmCitizensSinTarea.get(iZoneContainer);
                    if (alCits != null && alCits.size() > 0) {
                        // Hay aldeanos en la zona, le metemos la tarea si no hay otro con la misma
                        // tarea
                        boolean bCitizenConMismaTarea = false;
                        for (int c = 0; c < citizens.size(); c++) {
                            citizen = (Citizen) World.getLivingEntityByID(citizens.get(c));
                            if (citizen.getCurrentTask() != null
                                    && citizen.getCurrentTask().getTask() == Task.TASK_REMOVE_FROM_CONTAINER) {
                                // Aldeano con tarea similar, miramos si es la misma
                                if (Integer.parseInt(citizen.getCurrentTask().getParameter()) == container
                                        .getItemID()) {
                        ...
                        if (!bCitizenConMismaTarea) {
                            ...
                            int iIndexCit = getClosestCitizen(itemContainer.getCoordinates(), alCits,
                                    ActionPriorityManager.PRIORITY_HAUL);
                            if (iIndexCit != -1) {
                                citizen = alCits.remove(iIndexCit);
                                citizen.setCurrentTask(task);
                                iLibres--;
    ...
    private void assignHaulTasks(HashMap<Integer, ArrayList<Citizen>> hmCitizensSinTarea, int iLibres, int iHaulers) {
        // Las tareas de haul (put_in_container) usan algo de CPU, tampoco mucho (casi
        // nada de hecho), pero hay que recorrerse todos los items del mundo (árboles
        // inclusive)
        ...
        if (stockpiles.size() > 0 || containers.size() > 0) {
            ...
            // Vamos a recorrernos los items del mundo y ir viendo si está en una stockpile
            // válida, en caso negativo asignaremos a un aldeano
            // POPO Integer[] aItems = World.getItems ().keySet ().toArray (new Integer
            // [0]);
            Item item;
            ArrayList<Citizen> alCits;
            FocusData fd;
            ...
            break1: for (int i = haulIndex; i < MAX_INDEX; i++) {
                ...
                boolean bItemInTheCorrectPile = cell.hasStockPile()
                        && (Stockpile.getStockpile(cell.getCoordinates()).itemAllowed(item.getIniHeader()));
                if (bItemInTheCorrectPile) {
                    aItemsToDelete.add(i);
                    continue;
                }
                // boolean bItemCanBeStockpiled = !cell.hasStockPile () ||
                // (!Stockpile.getStockpile (item.getCoordinates ()).itemAllowed
                // (item.getIniHeader ()));
                ...
                if (alCits != null && alCits.size() > 0) {
                    // Hay aldeanos inactivos en esa zona, buscamos una stockpile/barril con puntos
                    // libres/espacio en esa zona
                    ...
                    for (int c = 0; c < citizens.size(); c++) {
                        citizen = (Citizen) World.getLivingEntityByID(citizens.get(c));
                        if (citizen.getCurrentTask() != null && (citizen.getCurrentTask().getTask() == Task.TASK_HAUL
                                || citizen.getCurrentTask().getTask() == Task.TASK_PUT_IN_CONTAINER)) {
                            // Citizen con tarea de haul/putInContainer, miramos si es la misma
                            if (item.getCoordinates().equals(citizen.getCurrentTask().getPointIni()) /*
                                                                                                      * && item.
                                                                                                      * getIniHeader
                                                                                                      * ().equals
                                                                                                      * (citizen.
                                ...
                                if (itemContainer != null && itemContainer.getCoordinates() != null) {
                                    if (World.getCell(itemContainer.getCoordinates()).getAstarZoneID() == iMatZoneID) {
                                        ...
                                        for (int c = 0; c < citizens.size(); c++) {
                                            citizen = (Citizen) World.getLivingEntityByID(citizens.get(c));
                                            if (citizen.getCurrentTask() != null
                                                    && (citizen.getCurrentTask().getTask() == Task.TASK_HAUL
                                                            || citizen.getCurrentTask()
                                                                    .getTask() == Task.TASK_PUT_IN_CONTAINER)) {
                                                // Citizen con tarea de mover en container, miramos si está llevando
                                                // algo a esa casilla
                                                if (itemContainer.getCoordinates()
                                                        .equals(citizen.getCurrentTask().getPointEnd())) {
                                                    // Ya lo está moviendo alguien
                                        ...
                                        if (!bCitizenHauling) {
                                            int iIndexCit = getClosestCitizen(item.getCoordinates(), alCits,
                                                    ActionPriorityManager.PRIORITY_HAUL);
                                            if (iIndexCit != -1) {
                            ...
                            if (!stockpile.isFull() && stockpile.itemAllowed(item.getIniHeader())) {
                                ...
                                for (int n = 0; n < points.size(); n++) {
                                    ...
                                    if (!cell.hasEntity() && cell.getAstarZoneID() == iMatZoneID) {
                                        // Tenemos casilla libre en la stockpile en la misma zona que hay aldeanos
                                        // inactivos, tarea que te vi al aldeano
                                        // que esté más cercano al item

                                        // Hay que mirar que no haya un aldeano MOVIENDO algo a esa casilla del
                                        // stockpile
                                        boolean bCitizenHauling = false;
                                        for (int c = 0; c < citizens.size(); c++) {
                                            citizen = (Citizen) World.getLivingEntityByID(citizens.get(c));
                                            if (citizen.getCurrentTask() != null
                                                    && (citizen.getCurrentTask().getTask() == Task.TASK_HAUL
                                                            || citizen.getCurrentTask()
                                                                    .getTask() == Task.TASK_PUT_IN_CONTAINER)) {
                                                // Citizen con tarea de haul, miramos si está llevando algo a esa
                                                // casilla
                    ...
                    if (!bStockEncontrada && World.getCell(item.getCoordinates()).hasStockPile()) {
                        ...
                        if (!bFound) {
                            ...
                            LivingEntity le;
                            Cell cellCit;
                            forcits: for (int c = 0; c < citIDs.size(); c++) {
                                    ...
                                    if (cellCit.getAstarZoneID() == iMatZoneID) {
                                        ...
                                        while (distancia < distanciaMAXCITS) {
                                            ...
                                            for (int x = (le.getX() - distancia); x <= (le.getX() + distancia); x++) {
                                                for (int y = (le.getY() - distancia); y <= (le.getY()
                                                        + distancia); y++) {
                                                    if (Math.abs((le.getX() - x)) == distancia
                                                            || Math.abs((le.getY() - y)) == distancia) { // Para que
                                                                                                         // sólo mire
                                                                                                         // puntos
                                                                                                         // exteriores
                                                                                                         // del radio
                                                        if (Utils.isInsideMap(x, y, le.getZ())) {
                                                            ...
                                                            if (cell.isEmpty() && !cell.isFlagOrders()
                                                                    && cell.getAstarZoneID() == iMatZoneID
                                                                    && LivingEntity.isCellAllowed(cell)) {
                                                                if (!cell.hasStockPile() || (Stockpile
                                                                        .getStockpile(cell.getCoordinates())
                                                                        .itemAllowed(item.getIniHeader()))) {
                                                                    // Punto libre encontrado
                                                                    int iIndexCit = getClosestCitizen(
                                                                            item.getCoordinates(), alCits,
                                                                            ActionPriorityManager.PRIORITY_HAUL);
                                                                    ...
                                                                    if (iIndexCit != -1) {
                                                                        ...
                                                                        citizen.setCurrentTask(task);
                                                                        iLibres--;
                                                                        hmCitizensSinTarea.put(
                                                                                Integer.valueOf(iMatZoneID), alCits);
                                                                        if (iLibres <= 0) {
                                                                            // No más aldeanos libres
                                                                            break break1; // Fin
                                                                        }
        ...
        while (iterator.hasNext()) {
            ...
            if (alCits != null) {
                ...
                for (int i = 0; i < alCits.size(); i++) {
                    cit = alCits.get(i);
                    if (cit != null && cit.getCitizenData() != null
                            && !cit.getCitizenData().containsDeniedJob(iHaulID)) {
                        iLibres++;
                    }
                }
            }
    ...
    public boolean setHotPointFinished(Task task, int hotPointIndex) {
        ...
        for (int i = 0; i < hpSize; i++) {
            if (!task.getHotPoints().get(i).isFinished()) {
                // Encontramos un HP NO acabado, devolvemos true para indicar que aún quedan
                // cosas por hacer
                return true;
            }
        }
